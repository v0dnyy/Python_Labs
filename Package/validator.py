...
class Validator:
    ...
    def check_inn(self) -> bool:
        ...
        Возвращается или True или False
        """
        if re.match(r"^\d{12}$", str(self._inn)) is not None:
            return True
        return False
    ...
    def check_age(self) -> bool:
        ...
        Возвращается или True или False
        """
        if re.match(r"[0-9]{1,3}", str(self._age)) is not None and 14 < int(self._age) <= 100:
            return True
        return False
...
def file_validate(input_pas: str, output_pas: str):
    data = File(input_pas).data
    count_valid = 0
    count_invalid = 0
    ...
    with open(output_pas, 'w', encoding='utf-8') as output:
        json.dump(list_result, output, indent=4, ensure_ascii=False)
    with open(output_pas, 'w', encoding='utf-8') as output_:
